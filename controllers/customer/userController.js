...
// ------------------- Send Mobile OTP (SMS + Email Support) -------------------
  ...
  try {
    ...
    try {
      const result = await sendOTPToBoth(
        mobile, 
        user.email, // Send to email if available
        otp, 
        "registration",
        user.name
      );
...
exports.resendOTP = async (req, res) => {
  try {
    ...
    // Send OTP to identifier + alternative contact if available
    try {
      const result = isEmail 
        ? await sendOTPToBoth(user.mobile, user.email, otp, "login", user.name)
        : await sendOTPToBoth(user.mobile, user.email, otp, "login", user.name);
...
// ------------------- Send Password Reset OTP (SMS + Email) -------------------
exports.sendPasswordResetOTP = async (req, res) => {
  try {
    ...
    // Send OTP via both SMS and Email if available
    try {
      const result = await sendOTPToBoth(
        user.mobile, 
        user.email, 
      ...
      console.error("Password reset OTP error:", otpError);
    }
